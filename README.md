...
- поиск организаций, площадок и игроков находится внутри `Игры`, а поиск тренеров открывается игроком из блока `Без тренера` / `Мои тренеры` на первой вкладке.

Игрок может напрямую добавить нескольких тренеров. После успешного добавления сразу показывается `Тренируется у {имя}`. Тренер получает уведомление `Новый игрок`, видит человека в `Моих игроках` и может написать. Отдельного подтверждения тренера нет. Источник истины — [`docs/TRAINER_RELATIONSHIPS.yaml`](docs/TRAINER_RELATIONSHIPS.yaml), поисковые области — [`docs/SEARCH_ARCHITECTURE.yaml`](docs/SEARCH_ARCHITECTURE.yaml).
...
- поиск и связи игрока с тренерами;
