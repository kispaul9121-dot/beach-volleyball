...
Нижнее меню неизменно: `Профиль · События · Чаты · Клубы · Настройки`.

- `Профиль` — первая вкладка с публичной идентичностью активного actor, ближайшими действиями и блоком тренеров игрока;
- `События` — игры, тренировки, турниры, сезоны, туры, поездки и кэмпы;
- `Клубы` — только клубы, площадки и корты;
- `Настройки` — архивы, платежи, actor-профили и служебные параметры;
- поиск тренеров открывается игроком из блока `Без тренера` / `Мои тренеры` на первой вкладке.

Связь с тренером подтверждаемая: до принятия показывается `Запрос отправлен`, после принятия — `Тренируется у {имя}`. Поддерживается несколько тренеров. Источник истины — [`docs/TRAINER_RELATIONSHIPS.yaml`](docs/TRAINER_RELATIONSHIPS.yaml), поисковые области — [`docs/SEARCH_ARCHITECTURE.yaml`](docs/SEARCH_ARCHITECTURE.yaml).
...
- конкретная цветовая палитра, accent color и вид selected-чипов ещё не утверждены и должны обсуждаться отдельно;
...
Пример: `Настройки → Мои турниры и сезоны`.
